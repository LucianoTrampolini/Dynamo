...
namespace Dynamo.Boekingssysteem
{
    ...
    public partial class App : Application
    {
        ...
        protected override void OnStartup(StartupEventArgs e)
        {
            ...
            if(File.Exists(@"c:\temp\DynamoSettings.txt"))
            {

                var file = File.OpenText(@"c:\temp\DynamoSettings.txt");
                var connectionString = file.ReadLine();
                var providerName = file.ReadLine();

                Configuration configuration = ConfigurationManager.OpenExeConfiguration(ConfigurationUserLevel.None);
                ConnectionStringsSection contring = configuration.ConnectionStrings;
                contring.ConnectionStrings.Remove("Dynamo");
                contring.ConnectionStrings.Add(new ConnectionStringSettings("Dynamo", connectionString, providerName));
