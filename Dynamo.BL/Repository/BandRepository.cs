    ...
    public class BandRepository: RepositoryBase<Model.Band>
    {
        ...
        protected override void HandleComplexPropertyChanges(Model.Base.ModelBase entityBase)
        {
            ...
            if (entity.BandTypeId == BandTypeConsts.Contract) 
            {
                ...
                foreach (var bandLid in entity.BandLeden)
                {
                    HandleChanges(bandLid);
                }
        ...
        {
            return currentContext.Bands.Include(band => band.BandLeden).Include(band => band.Contracten).Where(expression).ToList();
        }
        ...
        public List<Persoon> GetBandLeden(Model.Band band)
        {
            return currentContext.Bands.Where(x => x.Id == band.Id).SelectMany(b => b.BandLeden).ToList();
        }
    }
}
